...
class ReportController extends Controller
{
    public function transaction()
    {
        ...
        $transactions = Transaction::with('user', 'owner')->orderBy('id','desc')->paginate(getPaginate());
    ...
    public function ownerLoginHistory(Request $request)
    {
        if ($request->search) {
            $search = $request->search;
            $pageTitle = 'Owner Login History Search - ' . $search;
            $emptyMessage = 'No search result found.';
            $login_logs = UserLogin::where('owner_id', '!=', 0)->whereHas('owner', function ($query) use ($search) {
                $query->where('username', $search);
            })->orderBy('id','desc')->with('owner')->paginate(getPaginate());
            return view('admin.reports.owner_logins', compact('pageTitle', 'emptyMessage', 'search', 'login_logs'));
        }
        $pageTitle = 'Owner Login History';
        $emptyMessage = 'No owners login found.';
        $login_logs = UserLogin::where('owner_id', '!=', 0)->orderBy('id','desc')->with('owner')->paginate(getPaginate());
        return view('admin.reports.owner_logins', compact('pageTitle', 'emptyMessage', 'login_logs'));
    }

    public function ownerLoginIpHistory($ip)
    {
        $pageTitle = 'Login By - ' . $ip;
        $login_logs = UserLogin::where('user_ip',$ip)->where('owner_id', '!=', 0)->orderBy('id','desc')->with('owner')->paginate(getPaginate());
        $emptyMessage = 'No owners login found.';
        return view('admin.reports.owner_logins', compact('pageTitle', 'emptyMessage', 'login_logs','ip'));

    }

    public function emailHistory(){
        $pageTitle = 'Email history';
        $logs = EmailLog::with('user', 'owner')->orderBy('id','desc')->paginate(getPaginate());
